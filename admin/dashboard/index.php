...
$sql = "select * from browser where unique_id = ?";
$check = pdo_get_one_row($sql, $_SESSION['unique_id']);

if ($check == []) {
    $sql = "insert into browser (browser_name, unique_id) values(?,?)";
} else {
    $sql = "update browser set browser_name = ? where unique_id =?";
}

pdo_execute($sql, $ua['name'], $_SESSION['unique_id']);
